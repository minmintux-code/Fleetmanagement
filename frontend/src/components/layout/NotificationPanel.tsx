...
export const NotificationPanel: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  ...
  return (
    <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-lg bg-white dark:bg-slate-800 shadow-xl border border-slate-200 dark:border-slate-700 z-50 overflow-hidden">
      ...
      <div className="max-h-96 overflow-y-auto divide-y divide-slate-100">
        {notifications.length === 0 ? (
          <div className="p-6 text-center text-slate-500 dark:text-slate-400 text-sm">No notifications right now.</div>
        ) : (
          notifications.map((n) => (
            <div
              key={n.id}
              onClick={() => markAsRead(n.id)}
              className={`p-3.5 hover:bg-slate-50 dark:bg-slate-900/50 transition-colors cursor-pointer flex items-start space-x-3 ${
      ...
      <div className="p-2.5 bg-slate-50 dark:bg-slate-900/50 text-center border-t border-slate-100 dark:border-slate-700/50">
        <Link
          to="/notifications"
          onClick={onClose}
